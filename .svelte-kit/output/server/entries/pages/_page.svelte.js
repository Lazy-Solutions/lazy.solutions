...
function Home($$renderer) {
	...
	for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
		...
		$$renderer.push(`<article class="asset-card"><span class="asset-status">${escape_html(asset.status)}</span> <h3>${escape_html(asset.title)}</h3> <p>${escape_html(asset.body)}</p> <a class="button secondary" target="_blank" rel="noopener noreferrer sponsored"${attr("href", asset.href)}>Unity Asset Store</a></article>`);
	}
